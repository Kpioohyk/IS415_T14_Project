...
```{r}
sc_clust <- function(nb_mat, cluster_df, clust_num, col_name) {
  # Generate Weights
  ...
  clust <- skater(ekps.mst[,1:2], cluster_df, method = "euclidean", clust_num)
  
  groups_mat <- as.matrix(clust$groups)
  
  ekps.transformed <- cbind(ekps.transformed, as.factor(groups_mat)) %>%
  rename(`col_name`=`as.factor.groups_mat.`)
  
  qtm(ekps.transformed, col_name)
}
```
