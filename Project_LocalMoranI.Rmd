...
Then, we import csv file in [section 3.4](#aspatial) using **read_csv() function** of readr package (tidyverse).
...
For this project, it is important to learn about the government administrative region of Indonesia. In Indonesia, nationally one country, divided into **34 provinces** which includes East Kalimantan. 
...
We will rename the columns of dataframe "area" into English.
...
Notes: We can see that there are no NA values and all variables have the same number of observations. 


## 3.3 Plot of the Study Area
Using OpenStreetMap as the base, we head on the plot the study areas in East Kalimantan province.  
```{r , echo=TRUE, eval=TRUE, warning=FALSE, message=TRUE}
tm_shape(area)+
...
## 3.4 Importing aspatial data (PODES 2018 in CSV format) {#aspatial}
...
As we can see from the glimpse() function, all the different parts of PODES 2018 dataframe have different number of columns/variables, see Table 1 below.  
...
Table 1: Describing the number of records in each podes dataframe
...
What we will be doing now is to either recode the variable or to change the variable values, and we will only be **changing the column name at the end to simplify the process, see [section 3.5.1.2](#changename)**. Then, we will be keeping a vector of all the variable name in podes1 dataframe and will be setting the column names of the new tidy podes1 data frame with that variable name vector.


#### 3.5.1.1 Recode or change the variable 
...
#### 3.5.1.2 Changing the column names of the variables {#changename}
...
```{r , echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
podes4_final <- podes4 %>%
  select("SHAPE", "PROVINCE_ID", "PROVINCE", "CITY_ID", "CITY", "DISTRICT_ID", "DISTRICT", "VILLAGE_ID", "VILLAGE", "AVAILABILITY_OF_VILLAGE_INFORMATION", "USAGE_OF_VILLAGE_FINANCIAL_SYSTEM","VILLAGE_OWNED_ENTERPRISE_LAND", "VILLAGE_BUILDINGS","VILLAGE_MARKET","OTHER_VILLAGE_ASSETS","VILLAGE_MED_TERM_DEV_PLAN", "PERIOD_OF_RPJM_DEVELOP_YEAR_START", "PERIOD_OF_RPJM_DEVELOP_YEAR_END", "VILLAGE_GOV_WORK_PLAN_2018", "VILLAGE_REGULATION_NUMBER_2017", "NO_OF_HEADMAN_REGULATION_2017", "AVAILABILITY_OF_VILLAGE_COOPERATION_BETWEEN_VILLAGES_2018", "AVAILABILITY_OF_VILLAGE_COOPERATION_WITH_THIRD_PARTIES_2018", "TRANSPORTATION_INFRASTRUCTURE_DEVELOPMENT", "ENERGY_INFRASTRUCTURE", "INFORMATION_COMMUNICATION_INFRASTRUCTURE", "SANITATION_INFRASTRUCTURE", "EDUCATION_CULTURE_HEALTH_INFRASTRUCTURE", "TRADING_SERVICES_INFRASTRUCTURE", "FARMING_PRODUCTION_VILLAGE_SMALL_INDUSTRY_INFRASTRUCTURE", "RECREATION_TOURISM_INFRASTRUCTURE", "DISASTER_MGMT_ENV_CONSERVATION_INFRASTRUCTURE", "GOV_SUPPORT_INFRASTRUCTURE", "EDU_HEALTH_PUBLIC_CULTURE_EMPOWERMENT", "VILLAGE_RESIDENTIAL_NEIGHBOURHOOD_MGMT_EMPOWERMENT", "VILLAGE_TRANSPORTATION_MGMT_EMPOWERMENT", "RENEWABLE_ENERGY_DEVELOPMENT_EMPOWERMENT", "INFORMATION_COMMUNICATION_MGMT_EMPOWERMENT", "AGRI_BASED_PRODUCTIVE_BUSI_NON_AGRI_VILLAGE_SMALL_BUSI_MGMT_EMPOWERMENT", "NATURE_CONSERVE_AWARENESS_DISASTER_MGMT_EMPOWERMENT", "COMMUNITY_ROLE_IMPROVE_VILLAGE_GOVERNANCE_EMPOWERMENT","QUALITY_CAPACITY_OF_VILLAGE_COMMUNITY_EMPOWERMENT", "ESTABLISHMENT_DEVELOPMENT_VILLAGE_GOV_ENTERPRISE_EMPOWERMENT")
```
